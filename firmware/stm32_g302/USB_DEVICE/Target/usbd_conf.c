...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
  HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_FS, 0x40);   /* RX shared: 64 words */
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x20); /* EP0 IN ctrl: 32 words */
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 1, 0x40); /* EP1 IN mouse 64B: 64 words */
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 2, 0x10); /* EP2 IN keyboard 8B: 16 words */
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 3, 0x40); /* EP3 IN vendor 64B: 64 words */
  }
  return USBD_OK;
}
...
{
  static uint32_t mem[(sizeof(USBD_G302_HandleTypeDef)/4)+1];/* On 32-bit boundary */
  return mem;
}
