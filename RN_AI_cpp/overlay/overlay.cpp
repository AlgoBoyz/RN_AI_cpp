...
static std::wstring g_overlay_class;

static std::wstring RandomId(const wchar_t* prefix)
{
    wchar_t buf[40];
    swprintf(buf, 40, L"%s_%04x%04x", prefix,
...
bool CreateOverlayWindow()
{
    ...
    g_overlay_class = RandomId(L"WCls");
    std::wstring g_overlay_title = RandomId(L"WCap");

    WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L,
                      GetModuleHandle(NULL), NULL, NULL, NULL, NULL,
                      g_overlay_class.c_str(), NULL };
    ::RegisterClassEx(&wc);

    g_hwnd = ::CreateWindowEx(
        WS_EX_TOPMOST | WS_EX_LAYERED,
        wc.lpszClassName, g_overlay_title.c_str(),
...
void OverlayThread()
{
    ...
    if (!g_overlay_class.empty())
        ::UnregisterClass(g_overlay_class.c_str(), GetModuleHandle(NULL));
}
