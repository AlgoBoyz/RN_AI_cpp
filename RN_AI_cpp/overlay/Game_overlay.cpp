...
static void RandomWindowId(std::wstring& class_name, std::wstring& title)
{
    ...
    wchar_t buf[48];
    swprintf(buf, 48, L"Wnd_%08x%08x",
             static_cast<unsigned>(rand()),
             static_cast<unsigned>(rand()));
    class_name = buf;
    swprintf(buf, 48, L"WMsg_%04x",
...
bool Game_overlay::Impl::CreateWindowAndDevices()
{
    ...
    RandomWindowId(class_name_, window_title_);
...
void Game_overlay::Impl::DestroyWindowAndDevices()
{
    ...
    if (!class_name_.empty())
        UnregisterClassW(class_name_.c_str(), hinst);
}
