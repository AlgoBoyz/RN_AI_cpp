...
// Crop a sub-region from a BGRA RawImage at arbitrary coordinates.
static RawImage crop_bgra(const RawImage& src, int x, int y, int w, int h) {
    x = std::max(0, std::min(x, src.width - 1));
    y = std::max(0, std::min(y, src.height - 1));
    w = std::min(w, src.width - x);
    h = std::min(h, src.height - y);
    if (w <= 0 || h <= 0) return {};
    RawImage result(w, h, 4);
    int src_stride = src.width * 4;
    int dst_stride = w * 4;
...
static std::vector<CropRegion> parse_crop_regions(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--crop-region" && i + 1 < argc) {
            ...
            bool found = false;
            for (auto& r : regions) {
                if (r.id == (uint8_t)id) { r = {(uint8_t)id, x, y, w, h}; found = true; break; }
            }
            if (!found) regions.push_back({(uint8_t)id, x, y, w, h});
        }
    }
    return regions;
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--crop-size" && i + 1 < argc) { /* ignored, use --crop-region */ ++i; }
    ...
    while (g_running.load()) {
        ...
        RawImage full_frame = capturer.GetNextFrame();
        ...
        int fw = full_frame.width;
        int fh = full_frame.height;
        ...
        for (size_t i = 0; i < regions.size(); ++i) {
            auto& cr = regions[i];
            int rx = (cr.x < 0) ? (fw - cr.w) / 2 : cr.x;
            int ry = (cr.y < 0) ? (fh - cr.h) / 2 : cr.y;
            RawImage cropped = crop_bgra(full_frame, rx, ry, cr.w, cr.h);
        ...
        if (fps_counter % 100 == 0) {
            ...
            printf("[frame_sender] frame=%llu fps=%.1f cap=%.2fms enc=%.2fms regions=%zu\n",
                   (unsigned long long)stats.frames_sent, 100.0 / elapsed, cap_ms, enc_ms, regions.size());
